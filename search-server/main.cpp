...
public:
    ...
    // Обёртка для метода поиска с предикатом
    template <typename DocumentPredicate>
    ...
    // Обёртка для метода поиска по статусу
    ...
    // Обёртка для обычного метода поиска
    ...
    // Возвращает количество запросов без результата за последние сутки
...
private:
    ...
    // Добавляем результат запроса и обновляем очередь
    void AddRequest(int results_count) {
        ++current_time_;
        bool is_empty = (results_count == 0);
        ...
        // Удаляем старые запросы, старше чем 1440 минут
